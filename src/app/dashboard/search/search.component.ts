...
export class SearchComponent implements OnInit {
  ...
  async searchCollections(searchTerm: string): Promise<void> {
    try {
      ...
      channelsSnapshot.forEach(doc => {
        let channelData = doc.data(); let channelName = (channelData["name"] || '').toLowerCase(); let members = channelData["members"] || [];
  ...
  async loadAllData(): Promise<void> {
    try {
      ...
      channelsSnapshot.forEach(doc => {
        let channelData = doc.data(); let members = channelData["members"] || [];
