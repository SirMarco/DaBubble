  ...
  private setupConversationsListener(currentUserId: string) {
    ...
    this.unsubscribeConversationsSnapshot = onSnapshot(conversationsQuery, (querySnapshot) => {
      ...
      querySnapshot.forEach(docSnapshot => {
        let members = docSnapshot.data()['members'] as string[];
  ...
  */
  private setupUsersListeners(userIds: string[]): void {
    ...
    userIds.forEach(userId => {
      let userDocRef = doc(this.firestore, `users/${userId}`); let unsubscribe = onSnapshot(userDocRef, (userDoc) => {
        if (userDoc.exists()) {
          let user = { uid: userDoc.id, ...userDoc.data() } as UserLogged; let existingIndex = users.findIndex(u => u.uid === user.uid);
