...
export class MessageService {
  ...
  getMessagesWithUsers(channelId: string): Observable<Message[]> {
    return new Observable((observer) => {
      let messagesRef = collection(this.firestore, `channels/${channelId}/messages`); let messagesQuery = query(messagesRef, orderBy('created_at', 'asc'));
  ...
  getThreadMessagesWithUsers(channelId: string, messageId: string): Observable<Message[]> {
    return new Observable((observer) => {
      let threadRef = collection(this.firestore, `channels/${channelId}/messages/${messageId}/thread`); let threadQuery = query(threadRef, orderBy('created_at', 'asc'))
